...
Paste the ticket key (e.g. ATBT-12345), or type "create" to create one, or "skip" to continue.
> 
```

**If user pastes an existing key (e.g. `ATBT-12345`):**

Add a comment to the ticket via `mcp__mcp-atlassian__jira_add_comment`:
```
🗄️ Data governance fix — {tag}

Notebook: {NOTEBOOK_URL}
Rules addressed: {comma-separated list}
Generated by: cortex-data-governance skill
```

Then show: `✅ Comment added to {KEY}: {JIRA_URL}`

**If user types `create`:**

Detect the Jira project key from the squad name:

| Squad | Project key |
|---|---|
| `autobot-squad` | `ATBT` |
| `matrix-squad` | `MAT` |
| `intelligentoctopus-squad` | `IOC` |
| anything else | ask the user: "Which Jira project? (e.g. ATBT)" |

Create the ticket via `mcp__mcp-atlassian__jira_create_issue`:
- `project_key`: detected above
- `summary`: `[Data Governance] Fix UC metadata — {table_name}`
- `issue_type`: `Task`
- `description`: (markdown)
  ...
  ## Cortex score before fix
  {score}% — failing: {comma-separated failing rules}

  ## Fix
  Path A — Databricks notebook sent to Ziggy.
  Notebook: {NOTEBOOK_URL}

  ## Next steps
  - [ ] Ziggy runs the notebook
  - [ ] Verify score in Cortex after ~24h
  ```
...
Notebook: {NOTEBOOK_URL}
Jira: {KEY} — {JIRA_URL}   ← or "skipped" if user chose skip
